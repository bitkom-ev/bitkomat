...
function init() {
    // hide all: #result,
    $("#bitkomat-nrw, #start, #intro, #weight, #result,  #header-body, #bottom").hide();
...
function initOnclickCallbacks() {
    ...
    $('#btn-bitkomat-nrw-show-qa').off('click').click(function () {
        showModal('#QAModal', '#DATAModal', '#qa-modal-body');
    });
    $('#btn-bitkomat-nrw-show-data').off('click').click(function () {
    ...
    //$('#btn-bitkomat-nrw-parteien').off('click').click(function () {  createParties('#parteien-modal-body');    showModal('#parteienModal', '', '');    });
    ...
    $('#btn-bitkomat-nrw-skip-remaining-theses').off('click').click(function () {
        showResults('weight');
    });
    $('#btn-bitkomat-nrw-show-weight').off('click').click(function () {
        showResults('weight');
    });
    $('#btn-bitkomat-nrw-change-weight').off('click').click(function () {
        showResults('weight');
    });
    $('#btn-bitkomat-nrw-show-results').off('click').click(function () {
        showResults('result');
    });
...
function showModal(id, hide, load) {
    ...
    if (load) {
        ...
        if (ergebnis = load.match('data')) {
            file = "./datenschutzhinweise-nrw.html";
            if (_paq) {
                _paq.push(['trackEvent', 'datenschutz-nrw', 'show']);
            }
        }
        /**
        ...
        if (ergebnis = load.match('qa')) {
            file = "./faq-nrw.html";
            if (_paq) {
                _paq.push(['trackEvent', 'faq-nrw', 'show']);
            }
        }
...
function showResults(type = 'weight') {
    ...
    if (type === 'result') {
        ...
        $("#btn-bitkomat-nrw-show-weight").show(200).css("display", "inline-block");
...
`;
    ...
    if (type === 'result') {
        ...
        $("#btn-bitkomat-nrw-show-weight").show(200).css("display", "inline-block");
        $("#btn-bitkomat-nrw-show-results").hide();
        //console.log('hide button show results, show weight');
    }
    if (type === 'weight') {
        $("#text-result-edit").show(200).css("display", "inline-block");
        $("#btn-bitkomat-nrw-show-results").show(200).css("display", "inline-block");
        $("#btn-bitkomat-nrw-show-weight").hide()
...
function showBitkomat(status) {
    ...
    if (status = "edit") {
        window.location = "#top";
        $('#btn-bitkomat-nrw-skip-remaining-theses').fadeIn().addClass('inline');
    }
}

function showResult(type = 'weight') {
    $("#start, #bitkomat-nrw, #intro, #weight, #result, #header-body").hide();
    $("#" + type + "").fadeIn();
    animateBars();
...
// -' + name + '
        saveAs(imageData, now + '-bitkomat-nrw-ergebnis.png');
