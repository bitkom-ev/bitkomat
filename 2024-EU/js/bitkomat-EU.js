...
var thesis_id = 0;
var textResourcesInstance = new TextResources();
$(function () {
    translate();
    init();
    initHammer();
...
function translate() {
    for (var prop in textResourcesInstance) {
        var id = prop.replace(/_/g, '-');
        $('#' + id).html(textResourcesInstance[prop]);
    }
}

function init() {
    // hide all: #result,
    $("#bitkomat-EU, #start, #intro, #weight, #result,  #header-body, #bottom").hide();
    $.getJSON("config/data.json")
        ...
        .fail(function () {
            let textResources = new TextResources();
            $('#error-msg').html('<div class="alert alert-danger" role="alert">' + textResources.error_loading_config_file + '</div>');
        });
}
...
function initOnclickCallbacks() {
    ...
    $('#btn-bitkomat-EU-show-qa').off('click').click(function () {
        showModal('#QAModal', '#DATAModal', '#qa-modal-body');
    });
    $('#btn-bitkomat-EU-show-data').off('click').click(function () {
    ...
    //$('#btn-bitkomat-EU-parteien').off('click').click(function () {  createParties('#parteien-modal-body');    showModal('#parteienModal', '', '');    });
    ...
    $('#btn-bitkomat-EU-skip-remaining-theses').off('click').click(function () {
        showResults('weight');
    });
    $('#btn-bitkomat-EU-show-weight').off('click').click(function () {
        showResults('weight');
    });
    $('#btn-bitkomat-EU-change-weight').off('click').click(function () {
        showResults('weight');
    });
    $('#btn-bitkomat-EU-show-results').off('click').click(function () {
        showResults('result');
    });
...
function showModal(id, hide, load) {
    ...
    if (load) {
        let file = "";
        let ergebnis = load.match(/data/); // Use RegExp to perform matching.

        // Determine the file based on the content requirement and track the event.
        if (ergebnis) {
            file = "./datenschutzhinweise-EU.html";
            trackEvent('datenschutz-EU', 'show');
        } else if (load.match(/qa/)) {
            file = "./faq-EU.html";
            trackEvent('faq-EU', 'show');
        }

        // Load and inject the content if a file is specified.
        if (file) {
...
function unsetImportant() {
    ...
    $('#btn-important').text(textResourcesInstance.btn_make_thesis_double_weight);
}

function setImportant() {
    $('#btn-important').removeClass('btn-light');
    $('#btn-important').addClass('btn-success');
    $('#btn-important').text(textResourcesInstance.btn_thesis_has_double_weight);
}
...
function loadThesis() {
    ...
    $('#thesis-number').text(textResourcesInstance.thesis_number(currentThesis + 1));
...
function showResults(type = 'weight') {
    ...
    if (type === 'result') {
        ...
        $("#btn-bitkomat-EU-show-weight").show(200).css("display", "inline-block");
...
function addResultSummary(list, pointsForList, maxAchievablePoints, type) {
    ...
    var text_percentage = textResourcesInstance.achieved_points_text(pointsForList, maxAchievablePoints);
...
`;
    ...
    if (type === 'result') {
        ...
        $("#btn-bitkomat-EU-show-weight").show(200).css("display", "inline-block");
        $("#btn-bitkomat-EU-show-results").hide();
        //console.log('hide button show results, show weight');
    }
    if (type === 'weight') {
        $("#text-result-edit").show(200).css("display", "inline-block");
        $("#btn-bitkomat-EU-show-results").show(200).css("display", "inline-block");
        $("#btn-bitkomat-EU-show-weight").hide()
...
function statementOrDefault(statement) {
    if (statement === "") {
        return textResourcesInstance.default_text_no_statement;
    } else {
        return statement;
    }
}
...
function showBitkomat(status) {
    ...
    if (status = "edit") {
        window.location = "#top";
        $('#btn-bitkomat-EU-skip-remaining-theses').fadeIn().addClass('inline');
    }
}

function showResult(type = 'weight') {
    $("#start, #bitkomat-EU, #intro, #weight, #result, #header-body").hide();
    $("#" + type + "").fadeIn();
    animateBars();
...
function printImageFrom(selected) {
    ...
    html2canvas(document.querySelector(selected), {
        ...
        //let name = prompt(textResourcesInstance.pdf_print_name);
        let imageData = canvas.toDataURL('image/png')
// -' + name + '
        saveAs(imageData, now + '-bitkomat-EU-ergebnis.png');
        ...
        if (_paq) {
            ...
            _paq.push(['trackLink', now + '-bitkomat-EU-ergebnis.png', 'download'])
